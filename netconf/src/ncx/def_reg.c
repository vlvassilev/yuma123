...
/********************************************************************
* FUNCTION find_cfgapp
*   
* find an configuration application instance struct
*
* INPUTS:
		 ...
		 uint32        *h)
{
	...
	*h = h1;
    ...
    for (app = (def_cfgapp_t *)dlq_firstEntry(&own->ownht[h1]);
	 app != NULL;
	 app = (def_cfgapp_t *)dlq_nextEntry(app)) {
	if (app->hdr.nodetyp==DEF_NT_CFGAPP &&
	    !xml_strcmp(appname, app->hdr.key)) {
	    return app;
	}
    }
			...
			void *dptr)
{
    ...
    app = find_cfgapp(own, appname, len1, &h);
    if (app) {
	app->cfgdata.data[cfgid] = dptr;
	return NO_ERR;
    }

    /* else create a new def_defnode_t struct and initialize it */
    app = m__getObj(def_cfgapp_t);
    if (app == NULL) {
	return ERR_INTERNAL_MEM;
    }
    (void)memset(app, 0x0, sizeof(def_cfgapp_t));
    app->hdr.nodetyp = DEF_NT_CFGAPP;
    app->hdr.key = appname;
    app->cfgdata.datatyp = NCX_NT_APP;
    app->cfgdata.data[cfgid] = dptr;

    /* add the new defnode to the ownht */
    dlq_enque(app, &own->ownht[h]);
    return NO_ERR;
...
/********************************************************************
* FUNCTION def_reg_find_cfgapp
*   
* find an configuration application instance record
*
* INPUTS:
*    modname == registry module to find
*            == NULL if default namespace given and the module
*               is not known
*    appname == registry application to find
*    cfgid == cfg_id_t of the cfg to check
*
* RETURNS:
*    pointer to the cfg_app_t data node that is stored, or NULL if not found
*    or any error
*********************************************************************/
void *
			 ...
			 ncx_cfg_t cfgid)
{
	...
	/* check all the module records for the application record */

	return NULL;   /*******/
    }

    app = find_cfgapp(own, appname, xml_strlen(appname), NULL);
    if (app && app->cfgdata.data[cfgid]) {
	/* entry found and data type is correct */
	return app->cfgdata.data[cfgid];
    }
...
/********************************************************************
* FUNCTION def_reg_del_cfgapp
*   
* delete a config application hdr (or all pointers)
*
* INPUTS:
*    modname == registry module to find
*    appname == application name to find
