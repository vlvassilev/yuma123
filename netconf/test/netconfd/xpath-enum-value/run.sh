...
if [ "$RUN_WITH_CONFD" != "" ] ; then
  ...
  confdc -c test-xpath-enum-value.yang --yangpath /usr/share/yuma/modules/ietf -o test-xpath-enum-value.fxs
  NCPORT=2022
  NCUSER=admin
  NCPASSWORD=admin
  ...
  /usr/sbin/netconfd --module=./test-xpath-enum-value.yang --validate-config-only --startup=good.xml
  expected_fail "/usr/sbin/netconfd --module=./test-xpath-enum-value.yang --validate-config-only --startup=bad.xml"
fi
