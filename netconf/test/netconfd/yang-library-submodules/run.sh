...
if [ "$RUN_WITH_CONFD" != "" ] ; then
  ...
  confdc -c test-yang-library-submodules.yang --yangpath . -o test-yang-library.fxs
  NCPORT=2022
  NCUSER=admin
  NCPASSWORD=admin
  ...
  /usr/sbin/netconfd --module=./test-yang-library-submodules.yang --no-startup --superuser=$USER 2>&1 1>tmp/server.log &
  SERVER_PID=$!
fi
sleep 3
