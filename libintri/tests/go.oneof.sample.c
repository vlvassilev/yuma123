...
void testSetOneofToGo()
{
  ...
  GoString *errstr = malloc(sizeof(*(in)));
  ...
  acl_ACL_AddACE(in, epty, errstr);
  if (errstr->n > 0)
  {
    printf("add errstr= %s\n", errstr->p);

    free(errstr);
    errstr = malloc(sizeof(*(in)));
    acl_ACL_UpdateACE(in, epty, errstr);
    if (errstr->n > 0)
    {
      printf("upd errstr= %s\n", errstr->p);
    }
...
void testGetOneofToGo()
{
  ...
  acl_ACL_GetACEList(epty, out, errstr);
  if (errstr->n > 0)
  {
    printf("errstr= %s\n", errstr->p);
  }

  printf("\n List_Len = %d", out->List_Len);
  for (int i = 0; i < out->List_Len; i++)
  {
    ...
    case aclpb_ACEEntry_Param_Union_Options_IPv4:
      if (out->List[i]->Param.Param_IPv4 == NULL)
      {
        printf("\n");
        continue;
      }
    ...
    case aclpb_ACEEntry_Param_Union_Options_IPv6:
      if (out->List[i]->Param.Param_IPv6 == NULL)
      {
        printf("\n");
        continue;
      }
    ...
    case aclpb_ACEEntry_Param_Union_Options_Mac:
      if (out->List[i]->Param.Param_Mac == NULL)
      {
        printf("\n");
        continue;
      }
