...
if [ "$RUN_WITH_CONFD" != "" ] ; then
  ...
  confdc -c test-xpath-derived-from.yang --yangpath /usr/share/yuma/modules/ietf -o test-xpath-derived-from.fxs
  NCPORT=2022
  NCUSER=admin
  NCPASSWORD=admin
  ...
  /usr/sbin/netconfd --module=./test-xpath-derived-from.yang --no-startup --superuser=$USER 2>&1 1>tmp/server.log &
  SERVER_PID=$!
fi
sleep 3
