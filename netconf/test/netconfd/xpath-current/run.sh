...
if [ "$RUN_WITH_CONFD" != "" ] ; then
  ...
  confdc -c ../test-xpath-current.yang --yangpath /usr/share/yuma/modules/ietf -o test-xpath-current.fxs
  NCPORT=2022
  NCUSER=admin
  NCPASSWORD=admin
...
#  confd --verbose --foreground --addloadpath ${RUN_WITH_CONFD}/src/confd --addloadpath ${RUN_WITH_CONFD}/src/confd --addloadpath ${RUN_WITH_CONFD}/src/confd/yang --addloadpath ${RUN_WITH_CONFD}/src/confd/aaa --addloadpath .
  ...
  /usr/sbin/netconfd --module=/usr/share/yuma/modules/ietf/iana-if-type@2014-05-08.yang --module=./test-xpath-current.yang --no-startup --superuser=$USER 2>&1 1>tmp/server.log &
  SERVER_PID=$!
fi
sleep 3
